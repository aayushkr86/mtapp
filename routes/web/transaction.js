...
module.exports = {
  ...
  //  var teller = req.body.tellerID;
    ...
    var transdate = moment().format('YYYY-MM-DD, h:mm:ss a');
    //  if (moment(transdate, 'YYYY-MM-DD,HH:mm').isValid()){
    //  transdate = moment(transdate).format('YYYY-MM-DD, h:mm:ss a');
    // }else{
    //   transdate = undefined;
    // }
  ...
  var remittermsgenglish =`Welcome to MTapp!You have just sent  ${amount} ${currency} at ${bname} ${bsurName},code:${transactionNo}.Thank You`;

   var remittermsgfrench =`Bienvenu(e) chez  MTapp ! Vous venez d’envoyer  ${amount} ${currency}   à  ${bname} ${bsurName}, code : ${transactionNo}. Merci`;


 var beneficiarymsgenglish = `Welcome to MTapp!You have just received  ${amount} ${currency} of ${remitname} ${remitsurName},thank you to approach a point of payment.Thank You`;

 var beneficiarymsgfrench = `Bienvenu(e) chez  MTapp ! Vous venez de recevoir  ${amount} ${currency}   de ${remitname} ${remitsurName}, merci de vous rapprocher d’un point de paiement. Merci`;
...
var subject;
 ...
 console.log(transdate)
                         ...
                         Teller
                           ...
                           .exec(function(err, tellerData){
                             if(err){
                              ...
                              if(tellerData.language == 'english'){
                                 htmlMsg = remittermsgenglish;
                                 htmlMsg1 = beneficiarymsgenglish;
                                 subject = 'Your Transaction Deatils'

                              }else if (tellerData.language == 'french') {
                                htmlMsg = remittermsgfrench;
                                htmlMsg1 = beneficiarymsgfrench ;
                                subject = 'Votre transaction Deatils'

                              };
                              if(amount <= tellerData.currentbalance){
                                                                       ...
                                                                       res.send({message: 'There was an error sending the beneficiaryemail'});
                                                                         ...
                                                                         sms.send(remitmobile,htmlMsg,(err,result)=>{
                                                                           if(err){
                                                                             ...
                                                                             sms1.send(bmobile,htmlMsg1,(err,result1)=>{
                                                                               if(err){
                                                                                    return res.json({error : true , reason: err});
                                                                               } else{
                                ...
                                var b = tellerData.currentbalance;
   ...
   if(err) {
     ...
     var remitterpaidmsgenglish = `Welcome to MTapp!The  ${book.amount} ${book.currency} sent to  ${book.beneficiaryname} ${book.beneficiarysurName} have just been removed code:${book.transactionNumber}.Thank you`;
     var remitterpaidmsgfrench = `Bienvenu(e) chez  MTapp !  Les ${book.amount} ${book.currency}  envoyés à ${book.beneficiaryname} ${book.beneficiarysurName} viennent d’être retirés code : ${book.transactionNumber}. Merci`;

    var beneficiaryenglishmail = `Dear Customer,Your refernce transaction number: ${book.transactionNumber} and amount:  ${book.amount} ${book.currency} has been successfully: ${book.status} `;
    var beneficiaryfrenchhmail = `Cher client, votre numéro de transaction de référence: ${book.transactionNumber} et le montant:  ${book.amount} ${book.currency} a été effectué avec succès: ${book.status}`;
     Teller
      .findOne({_id:book.tellerID})
      .exec(function(err , tellerdetails) {
         if(err){
           return res.json({error : true , reason: err});
         }

         if(tellerdetails.language == 'english'){
            htmlMsg = remitterpaidmsgenglish;
            htmlMsg1 = beneficiaryenglishmail;
            subject = 'Your Transaction Number and Status'
         }else if (tellerdetails.language == 'french') {
           htmlMsg = remitterpaidmsgfrench;
           htmlMsg1 = beneficiaryfrenchhmail;
           subject = 'Votre numéro de transaction et votre statut'
         };
                                       ...
                                       if (err) {
                                         ...
                                         sms.send(rmobile,htmlMsg,(err,result)=>{
                                           if(err){
                                             ...
                                             var sms1 = new BulkSMS('bramsdiouf','Saliou11diouf@');
                                             sms1.send(bmobile,htmlMsg1,(err,result)=>{
                                               if(err){
                                                    return res.json({error : true , reason: err});
                                               } else{
  ...
  getreimbursetransactionbybranch: function (req,res){
     ...
     dateRange: function(req,res){
       ...
       var end_date = moment(b, 'YYYY-MM-DD').add(1, 'day').format();
       Transaction
     ...
     findAll: function(req,res){
      ...
      var max = req.body.maxAmount;
       ...
       if(req.body.tellerID != undefined){                                          /*FIND BY tellerID*/
         query["tellerID"] = req.body.tellerID;
         // Teller
