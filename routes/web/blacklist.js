         ...
         if(results.length){
          ...
          black.save(function (err, result){
                   if(err){
                        ...
                        Remitter.findByIdAndUpdate({_id : req.params.id}, { $set: {blacklist:"true"} }, function(err, raw) {
                         if (err) return res.json({error: true, reason: err, message: 'Blacklisted not possible'})
                            return res.json({error : false , result : result, message:'Blacklisted'});
                        });
                       }
          ...
          .findByIdAndRemove({_id : id}, function(err,result){
            if(err){
             return res.json({error : true , reason: err});
                  ...
                  Remitter.findByIdAndUpdate({ _id : req.params.id}, { $set: {blacklist:""} }, function(err, raw) {
                   if (err) return res.json({error: true, reason: err, message: 'Remove from Blacklisted not possible'})
