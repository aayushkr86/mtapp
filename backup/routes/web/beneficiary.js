...
findbenef: function(req,res){
    ...
    var country = b64url.decode(req.body.country);
    Beneficiary
       .findOne({'beneficiaryemail': mail, 'beneficiaryname':name ,'beneficiarysurName':surName,'country':country })
       .exec(function (err, result) {
         if(err){
           return res.json({error: true , reason: err});
         }
              ...
              .exec(function (err, country) {
                ...
                if(result.amount <= country.paymentLimit){
                    Beneficiary.findOneAndUpdate({beneficiaryemail:result.beneficiaryemail}, {$set:{status:'Verified'}},{new: true},function(err, doc){
                      ...
                      return res.json({message:'Beneficiary amount has less than country paymentlimit',result:doc});
                  });

                } else{
                    Beneficiary.findOneAndUpdate({beneficiaryemail:result.beneficiaryemail}, {$set:{status:'Blacklist'}},{new: true},function(err, doc){
                      if(err){
                          return res.json({error : true, reason : err});
                      }
