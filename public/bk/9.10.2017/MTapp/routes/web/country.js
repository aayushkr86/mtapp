...
add:function(req,res){
  ...
  if(!country){
           ...
           return res.json({error : false , message : 'Currency can not be empty'});
            ...
            return res.json({error : false , message : 'CountryLimit can not be empty'});
        }else if(!region){
            return res.json({error : false , message : 'Region can not be empty'});
        }else{
 Country.find({countryName:country}).exec(function (err, results) {
     if(err){}
   if(results.length){
       return res.json({error : false , message : 'This countryLimit already define,Please choose another country'});
   }else{
     ...
     teller.save(function (err, result){
              if(err){
                  ...
                  return res.json({error : false , message : 'CountryLimit added' ,result:a});
              });
   }
