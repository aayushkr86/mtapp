...
getalltellerbybranch: function(req,res){
  Teller
...
res.mailer.send('passvisior', {
    ...
    .then(function (savedata) {
      ...
      res.mailer.send('passvisior', {
              ...
              if (err) {
                ...
                console.log(savedata.paymentLimit);
                console.log(savedata.tellerID);
                TellerID
 ...
 delete: function(req,res){
     ...
     if(err){
         return res.json({error : true , reason: err});
           ...
           if(result.isActive === true && !result.upcomingStatus){
             Teller.findByIdAndUpdate({ _id: req.params.id }, { $set: { upcomingStatus: "false" } }, function(err, raw) {
               if (err){
                 console.log(err)
                  return res.json({error: true, reason: err, message: 'The status of tellerid request cannot be updated'})
                }
                console.log(raw)
               return res.json({ error: false, message: 'status of tellerid updated,after checker approval' , upcomingStatus: raw.upcomingStatus });
             })
           }else if (result.isActive === false && !result.upcomingStatus) {
             Teller.findByIdAndUpdate({ _id: req.params.id }, { $set: { upcomingStatus: "true"} }, function(err, raw) {
               if (err){
                 console.log(err)
                  return res.json({error: true, reason: err, message: 'The status of tellerid request cannot be updated'})
                }
  ...
  tellerStatusfinalupdate: function(req,res){
       ...
       if(err){
           return res.json({error : true , reason: err});
            ...
            console.log(currentstatus)
             Teller.findByIdAndUpdate({ _id: req.params.id }, { $set: {isActive: currentstatus,upcomingStatus:""} }, function(err, raw) {
               if (err) return res.json({error: true, reason: err, message: 'The status of tellerid request cannot be updated'})
               console.log(raw)
               return res.json({ error: false, message: 'status of tellerid updated' , isActive: currentstatus });
             });
          }
     });
   }; //if
...
tellerchecker: function(req,res){
    Teller
...
updatelanguage: function(req,res){
  ...
  Teller.findByIdAndUpdate({ _id: req.params.id }, { $set: {language: req.body.language} }, function(err, result) {
    if (err){
      console.log(err)
       return res.json({error: true, reason: err, message: 'Cannot be updated'});
     }
