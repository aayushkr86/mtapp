...
function($timeout,$base64,$http,$window,$scope,$location,$translate){
  ...
  if($scope.tellerlogdata == 'undefined' || $scope.tellerlogdata == '' || $scope.tellerlogdata == null){
      ...
      console.log('token',$scope.tellertokendata);
    ...
    $http({
     method: 'GET',
     url: '/getcurrentbalance/'+$scope.tellerid,
     }).success(function (response) {
    $scope.currentbalance = response.result;
  ...
  /* ====================laguage change function =================*/
    $scope.frenchlanguage = function(){
      var frenchdata ={
       "language" : "french"
      };
      $http({
         method: 'PUT',
         url: '/updatelanguage/'+$scope.tellerid,
         data: frenchdata,
         }).success(function (response) {
          console.log('frenchlang',response)

         }).error(function (response) {
            console.log('frenchlangerror',response)
         })
    };
    $scope.englishlanguage = function(){
      var englishdata ={
       "language" : "english"
      };
      $http({
         ...
         data: englishdata,
          ...
          console.log('englishlang',response)

         }).error(function (response) {
            console.log('englishlangerror',response)
         })
    };
  /*==================== end change language function ================*/
/*====end transalator ==*/
  ...
  setTimeout(function(){
   ...
   /*change*/
    ...
    /*prateek*/
        ...
        console.log('tr',$scope.tariffRecord)
        $http({
             method: 'POST',
        ...
        console.log('ta',$scope.datacountry)
        $http({
             method: 'POST',
   ...
   $scope.createTransaction = function(){
    ...
    $scope.bmobile1 = "+"+$scope.beneficiarymobile
        var date = new Date();
$scope.transactiondate = date.getFullYear() + '-' + ('0' + (date.getMonth() + 1)).slice(-2) + '-' + ('0' + date.getDate()).slice(-2)+ ','+('0' + date.getHours()).slice(-2) + ":" 
                        + ('0' + date.getMinutes()).slice(-2);

       console.log('ttt',$scope.transactiondate)
      ...
      "tellerID":$scope.tellerID
       ...
       console.log('trnsdata',$scope.transactionData)
       ...
       $http({
            ...
            data: $scope.transactionData,
              ...
              if(response.message == 'Transaction done'){
                ...
                $scope.totaltax = null;
                 ...
                 $scope.fees =null;
                  ...
                  $scope.todaytransactiondata ={ 
                    "tellerID":$base64.encode($scope.tellerid)
                  };
                   $http({
                       method: 'POST',
                       url: '/tellerowntransaction',
                       data:$scope.todaytransactiondata
                       }).success(function (response) {
                       
                        $scope.todaytransactionDetails = response.result1.reverse();

                            $scope.currenttodayPageReport = 0;
                             $scope.pageSizetodayReport = 10;
                             $scope.numberOfPagestodayReport=function(){
                               return Math.ceil($scope.todaytransactionDetails.length/$scope.pageSizetodayReport);
                             } 
...
$scope.createTransactionhide = function(){
  var createTransactionbtn = document.getElementById('createTransactionbtn');
  createTransactionbtn.style.display = "none"
    ...
    $scope.kycCheck = function(){
      if(!$scope.remittername){
        new PNotify({
         tittle : 'Regular notice',
         type :'error',
         text :"Remitter name can not be blank",
       });
      }else if (!$scope.remittersurName) {
       new PNotify({
         tittle : 'Regular notice',
         type :'error',
         text :"Remitter surname can not be blank",
       });
      }else{
      $scope.rname = $base64.encode(encodeHtmlEntity(addslashes($scope.remittername)));
      $scope.rsurname = $base64.encode(encodeHtmlEntity(addslashes($scope.remittersurName)));
      ...
      $http({
          ...
          if(response.message == 'Verified'){
                var createTransactionbtn1 = document.getElementById('createTransactionbtn');
                createTransactionbtn1.style.display = "inline-block"
          }
    ...
    $scope.cancelTransaction = function(){
      ...
      $scope.totaltax = null;
       ...
       $scope.fees =null;
       var createTransactionbtn1 = document.getElementById('createTransactionbtn');
       createTransactionbtn1.style.display = "none"
    };
    $(document).ready(function(){
      $( function() {
         $( "#datepickerremiterissue" ).datepicker({ dateFormat: 'yy-mm-dd' });
       } );
    });
    $(document).ready(function(){
    $( function() {
       $( "#datepickerremiterexp" ).datepicker({ dateFormat: 'yy-mm-dd' });
     } );
    });
    ...
    $scope.searchTransaction = function(){
      ...
      $http({
       ...
       $(document).ready(function(){
          $( function() {
             $( "#datepickerbeneficiaryissue" ).datepicker({ dateFormat: 'yy-mm-dd' });
           } );
        });
        $(document).ready(function(){
        $( function() {
           $( "#datepickerbeneficiaryexp" ).datepicker({ dateFormat: 'yy-mm-dd' });
         } );
        });
...
$http({
     method: 'GET',
     // url: '/getsupplierbyTeller/'+$scope.tellerid,
     url:'/supplierController'
     }).success(function (response) {
       $scope.supplierDetails = response.result1.reverse();

           $scope.currentPageSupplier = 0;
           $scope.pageSizeSupplier = 10;
           $scope.numberOfPagesSupplier=function(){
             return Math.ceil($scope.supplierDetails.length/$scope.pageSizeSupplier);
           }
   }).error(function (response) {
      new PNotify({
         tittle : 'Regular notice',
    ...
    /*========  paying supplier =====================*/
     $(document).ready(function(){
        $( function() {
           $( "#datepickerinvoice" ).datepicker({ dateFormat: 'yy-mm-dd' });
         } );
      });
  ...
  $scope.$watch("region", function(newValue, oldValue) {
       $timeout(function() {
         ...
         $scope.region = newValue;
          $("#region1").material_select();
          $('.caret').text("");
          $("#region1").val(newValue);


       }, 0);
   });
  $scope.$watch("countrysearch", function(newValue, oldValue) {
       $timeout(function() {

         $scope.countrysearch = newValue;
          $("#countrysearch1").material_select();
          $('.caret').text("");
          $("#countrysearch1").val(newValue);
