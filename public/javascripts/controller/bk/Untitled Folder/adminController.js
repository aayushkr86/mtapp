app.controller('adminController',['$location','$timeout','$base64','$http','$window','$scope','$translate', function($location,$timeout,$base64,$http,$window,$scope,$translate){
   ...
   if($scope.admindata == 'undefined' || $scope.admindata == '' || $scope.admindata == null){
     ...
     $scope.decoded = jwt_decode($scope.admindata);
      ...
      $scope. usernamepattern = /^[a-zA-Z, -,0-9,`,~,!,@,#,$,%,^,&,*,(,),°]{2,30}$/;
       $scope.address = /^[a-zA-Z, -,0-9,`,~,!,@,#,$,%,^,&,*,(,),°]{1,200}$/;
      // $scope.emailpattern = /^[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,64}/;
      $scope.workingpattern = /^[0-9]{1,2}$/;
      $scope.workingdays =  /^[a-zA-Z, -,0-9,`,~,!,@,#,$,%,^,&,*,(,),°]{3,100}$/;
...
$http({
   ...
   $scope.addBranch = function(){
    ...
    $scope.getvalue();
     ...
     $scope.branch = $scope.branchID;
     $scope.countr = $scope.country;
     $scope.full = $scope.fullName;
     $scope.Add1 = $scope.Address1;
     $scope.Add2= $scope.Address2;
...
$scope.deleteBranch = function(branch){
  $http({
   ...
   $scope.createSupervisior = function(){
     ...
     $scope.branchid = $scope.supbranchID;
     // $scope.country = $base64.encode($scope.superCountry);
     $scope.full = $scope.superName;
     $scope.Address = $scope.superAddress;
...
$scope.deleteController = function(control){
  $http({
   ...
   $scope.createCountry = function (){
    ...
    $scope.countryname = $scope.countrylimit;
    $scope.countrycurrency = $scope.currency;
    $scope.paylimit = $base64.encode($scope.monthlylimit);
    $scope.countryregion = $scope.region;
    console.log('region',$scope.countryregion)
...
$http({
  ...
  /*============== createCountryCurrency========================*/
    ...
    $scope.createCountryCurrency = function(){
      console.log(escape($scope.countryName))
    $scope.currencydata = {
      "countryName":$scope.countryName ,
    ...
    $http({
        ...
        headers: {'Content-Type': 'application/json'
             ...
             if(response.result == 'countrycurrecny added'){
              new PNotify({
                 tittle : 'Regular notice',
                 type :'success',
                 text :response.result,
              ...
              if(response.result == 'countryName can not be empty'){
              new PNotify({
                 tittle : 'Regular notice',
                 type :'error',
                 text :response.result,
             ...
             if(response.result == 'currency can not be empty'){
              new PNotify({
                 tittle : 'Regular notice',
                 type :'error',
                 text :response.result,
              ...
              if(response.result == 'Country and currency already defined'){
              new PNotify({
                 tittle : 'Regular notice',
                 type :'error',
                 text :response.result,
